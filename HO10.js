...
if (stringPassword.length < 8) //if statement that requires the password to be more than 8 characters  
{
    stringOutput = "The password you entered is too short.";
}
else //checks to see if the password is in the bad passwords array
{
    for (var a=0; a < arrayBadPasswords.length; a++)
    {
       if (stringPassword === arrayBadPasswords[a]) 
       {
          stringOutput = "The password you entered is bad"; //if the password is in the bad passwords array then it returns this message
       }
    }
}
    if(stringOutput != "")
    {
        $("output").value= stringOutput;  //If the password triggered IF statement, and the output doesn't equal " " than the string output is given
    }
    else
    {
        $("output").value= "The password you entered is good"; 
    }  
};
